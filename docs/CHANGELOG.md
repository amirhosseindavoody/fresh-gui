...
- Each PTY child gets `TERM=xterm-256color` and `COLORTERM=truecolor`. Fish is started with `-i`. The pane resizes the grid (about 8×18px cells). The wheel scrolls history, and on the alternate screen sends up/down. Shift+PageUp / Shift+PageDown scroll history. Mouse selection and OSC 52 clipboard are still absent. Fish does not get the bash/zsh OSC 7 hook.
