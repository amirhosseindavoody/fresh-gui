...
| Workspace restore | Tabs come back in saved order with the saved active tab selected. A terminal whose shell ended with the previous daemon reopens as a new shell in the workspace root with its old title. The tab list is saved on every tab, folder, and focus change, on Disconnect / Reconnect, and on window close. Dock split geometry is not saved |
...
| PTY I/O (`alacritty_terminal` grid) | Yes. Cursor, SGR colors, and the alternate screen (vim, htop, fish). The grid follows the pane (about 8×18px cells). Wheel scrolls history; on the alternate screen it sends up/down. Shift+PageUp / Shift+PageDown scroll history. Drag selects text; `Ctrl+C` / `Cmd+C` or **Copy** (right-click or **···**) copies it, and a click or typing clears it. OSC 52 copy sets the host clipboard (256 KiB cap); OSC 52 paste is ignored. New shells start in the workspace or remote root. Bash, zsh, and fish emit OSC 7 so the next shell follows `cd` |
