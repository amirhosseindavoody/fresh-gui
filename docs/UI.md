...
Primer-inspired dark/light surfaces live in `src/tokens.css` (palette `primer`). Named packs in `src/palettes.ts` reuse Fresh editor theme colors (`vendor/fresh/.../themes/*.json`) mapped onto the same CSS variables for chrome, xterm (`xtermThemeFromCss`), and CodeMirror. Tailwind `@theme` in `styles.css` exposes them as utilities (`bg-bg`, `text-muted`, …).
