...
| PTY I/O (`alacritty_terminal` grid) | Yes. Cursor, SGR colors, and the alternate screen (vim, htop, fish). The grid follows the pane (about 8×18px cells). Wheel scrolls history; on the alternate screen it sends up/down. Shift+PageUp / Shift+PageDown scroll history. New shells start in the workspace or remote root. No mouse selection, no OSC 52 clipboard |
