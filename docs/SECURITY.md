...
| **Saved workspace list names project paths** | `workspaces.json` holds workspace names, absolute roots, open editor paths, terminal titles, and open explorer folders so the list survives a daemon restart. It holds no token and no file contents. Unix: `$XDG_STATE_HOME/fresh-gui/workspaces.json` (file `0600`, directory `0700`). Windows: `%LOCALAPPDATA%\fresh-gui\workspaces.json` (user profile ACLs). Written via a private temp file and an atomic rename. On load, each saved root goes back through `fs_authorize` (canonical, must be an existing directory), so the file cannot widen the sandbox beyond what an authenticated client could already authorize. Delete the file to forget every workspace. |
| **Symlinks in the explorer** | A listed symlink reports `target_kind` only when its canonical target is inside the FS root or an authorized directory. A link that leaves the sandbox stays a plain symlink row: it is not expandable, and listing or opening it is still refused by the same root check. |
| **Git commands** | `git` runs on the daemon in the workspace root (or the FS root when that workspace has none). Paths from the client must be repo-relative: no `..`, no absolute path, no NUL. Prompts are disabled (`GIT_TERMINAL_PROMPT=0`), so a missing credential fails instead of hanging the daemon. Output is capped for diffs (256KB per side). |
| **Open externally** | `fs_open_external` resolves the path through the same FS sandbox as other file operations, then spawns the OS opener (`start` on Windows, `open` on macOS, `xdg-open` elsewhere). File bytes are not sent to the client. |
| **OSC 52 clipboard** | A program in the PTY can set the host clipboard with OSC 52 (copy). Payloads over 256 KiB are dropped. OSC 52 paste/read is not answered, so that program cannot read the host clipboard. |
