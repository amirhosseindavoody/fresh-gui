...
| Open a file | Click a file in the explorer (or `Ctrl+P` → path `[:line[:col]]`). Folders use folder icons and a click expands or collapses only that folder; files use a type icon. A changed file in a Git repo opens a split diff; click it again to keep that diff as a tab. Right-click **Open Editor** opens the text buffer. A binary file opens a placeholder with **Open externally** |
| Source Control | Activity-bar Git icon (when the daemon advertises `git`). Lists changed and staged files for the workspace folder. Stage, unstage, commit, pull, and push run `git` on the daemon |
| File menu | **Stop Server** disconnects and stops a local daemon (`fresh-gui close`). **Quit Client** closes the window and leaves the daemon running. On an SSH session, Stop Server only disconnects |
...
The product host is this GPUI client. The daemon is a headless ADE WebSocket. A later browser host would be this same GPUI UI via WebAssembly, which is not in this release. Splits and tab reorder use the gpui-component dock. The in-app file clipboard lasts for the client session. The **File** menu sits in the title bar. Source Control is the activity next to the explorer when the connected daemon includes `git`. Diff tabs are for the current session only; they are not restored after a restart. Shell tabs show a cursor, colors, and full-screen programs (vim, htop, fish). Drag to select text; `Ctrl+C` / `Cmd+C` copies the selection (with nothing selected, `Ctrl+C` still interrupts). Full-screen programs that turn on mouse tracking receive those clicks and the wheel; Shift+drag still selects on the host. A program in the shell can set the clipboard with OSC 52; it cannot read it. A new shell starts in the workspace folder, or in the remote session root, rather than the daemon's home directory. Bash, zsh, and fish report the directory after `cd` (OSC 7). The wheel and Shift+PageUp / Shift+PageDown scroll history. Still absent: markdown WYSIWYG, a minimap, dock split restore, find, palette packs, tab pin, editor-tab rename, and hunk-level or branch Git operations. Copy Path and in-app paste use absolute ADE paths. On this GPUI snapshot, Linux clipboard writes do not offer `text/uri-list`, so pasting into a file manager is not reliable; paste inside the explorer is a real `fs_copy`.
